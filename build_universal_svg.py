...
def build_universal_svg(email: str, default_class: str = "idle_0") -> str:
    ...
    for vowel in ["a", "e", "i", "o", "u"]:
        vowel_file = (
            assets_path / "mouths" / "open" / f"emote_vowel_{vowel}_{open_mouth_idx + 1}.svg"
        )
        if vowel_file.exists():
            mouth_groups[f"vowel_{vowel}"] = (
                f'  <g class="vowel_{vowel}">{read_asset_content(vowel_file)}</g>'
            )
    ...
    # Build CSS rules
    css_rules = [
        f"#avatar-{avatar_id} .eyes > g, #avatar-{avatar_id} .mouths > g {{ display: none; }}"
    ]
...
if __name__ == "__main__":
    ...
    print(
        f"Difference:         {current_total-size:>8,} bytes ({(current_total-size)/1024:>6.2f} KB)"
    )
    ...
    print(
        f"Universal gzipped:  {universal_gzip_size:>8,} bytes ({universal_gzip_size/1024:>6.2f} KB)"
    )
