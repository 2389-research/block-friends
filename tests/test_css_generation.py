...
def test_css_rules_for_idle_frames():
    ...
    assert (
        f"#{avatar_id}.idle_0 .eyes>.open,#{avatar_id}.idle_0 .mouths>.closed{{opacity:1}}" in css
    )

    # Check idle_2 rule (closed eyes, bored mouth)
    assert (
        f"#{avatar_id}.idle_2 .eyes>.closed,#{avatar_id}.idle_2 .mouths>.bored{{opacity:1}}" in css
    )
...
def test_css_rules_for_emotes():
    ...
    for emote in ["happy", "sad", "surprised", "angry", "bored"]:
        ...
        assert (
            f"#{avatar_id}.{emote} .eyes>.{emote},#{avatar_id}.{emote} .mouths>.{emote}{{opacity:1}}"
            in css
        )
...
def test_css_rules_for_vowels():
    ...
    for vowel in ["a", "e", "i", "o", "u"]:
        ...
        assert (
            f"#{avatar_id}.vowel_{vowel} .eyes>.open,#{avatar_id}.vowel_{vowel} .mouths>.vowel_{vowel}{{opacity:1}}"
            in css
        )
