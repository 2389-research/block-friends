...
def test_generate_all_universal_states(generator, tmp_path):
    ...
    for state in states:
        assert (
            f".{state} " in svg_universal or f".{state}}}" in svg_universal
        ), f"State {state} not found in CSS"
...
def test_legacy_mode_all_frames(generator, tmp_path):
    ...
    for frame in frames:
        ...
        assert (
            '<g class="eyes"' not in svg_legacy
        ), f"Legacy frame {frame} should not have nested eyes group"
