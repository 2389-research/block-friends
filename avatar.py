...
def generate_avatar(
    input_string: str, config: DoorAgentConfig, generator: DoorAgentGenerator
) -> tuple:
...
Examples:
  ...
  python avatar.py "alice@wonderland.org" --output custom.svg  # Custom output path
    ...
    parser.add_argument(
        "input", help="Input string (typically email address) to generate avatar from"
    )
    parser.add_argument(
        "-o", "--output", help="Output SVG file path (default: out/avatar/[hash].svg)"
    )
    parser.add_argument(
        "--dry-run", action="store_true", help="Output SVG to stdout instead of saving to file"
    )
    parser.add_argument(
        "-i", "--info", action="store_true", help="Show agent configuration details"
    )
    parser.add_argument(
        "-j",
        "--json",
    ...
    try:
        ...
        if args.info:
            if args.json:
                ...
                print(
                    f"   Mouth: #{agent_config['mouth_index']} {'(excited)' if agent_config['excited'] else '(rest)'}",
                    file=sys.stderr,
                )
                ...
                print(
                    f"   Feet: {'Match body' if agent_config['feet_match_body'] else 'Match nodes'} ({agent_config['feet_color']})",
                    file=sys.stderr,
                )
